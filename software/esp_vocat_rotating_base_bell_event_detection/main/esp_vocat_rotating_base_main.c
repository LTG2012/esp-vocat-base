...
static const char *TAG = "ESP_VoCat Rotating Base";
...
/**
 * @brief Magnetic slide switch event callback function
 * 
 * @param event Detected magnetic slide switch event
 * 
...
static void magnetic_slide_switch_event_cb(magnetic_slide_switch_event_t event)
{
    ...
    esp_err_t ret = control_serial_send_magnetic_switch_event((uint16_t)event);
    if (ret != ESP_OK) {
...
void app_main(void)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to register magnetic slide switch callback: %s", esp_err_to_name(ret));
    } else {
